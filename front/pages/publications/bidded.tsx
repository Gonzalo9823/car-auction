...
import { useBiddedPublications } from '../../hooks/useBiddedPublications';
import Link from 'next/link';
import { User } from '../../interfaces/User';

interface PublicationsBiddedPageProps {
  isLogged: boolean;
  user?: User;
}

const PublicationsBidded: NextPage<PublicationsBiddedPageProps> = ({ isLogged, user }) => {
  const { publications } = useBiddedPublications();
    ...
    <Layout isLogged={isLogged}>
      <div className="bg-white">
        <div className="mx-auto max-w-2xl pb-16 pt-8 px-4 sm:pb-24 sm:pt-10 sm:px-6 lg:max-w-7xl lg:px-8">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900">Mis Ofertas</h2>
                ...
                <div className="mt-4 flex justify-between">
                  <div>
                    ...
                    <h3 className="text-sm mt-1 text-gray-700">
                      <Link href={`/publications/${publication.id}`}>
                        <span aria-hidden="true" className="absolute inset-0" />
                        Mi Oferta: {publication.bids.find((bid) => bid.bidder.id === user?.id)?.amount || '-'}
                      </Link>
                    </h3>
...
export default PublicationsBidded;
